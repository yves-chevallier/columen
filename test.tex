...
\newcount\WI@activecount
\newif\ifWI@active
\WI@activefalse

\newcommand\WI@activate{%
  \global\advance\WI@activecount\@ne
  \global\WI@activetrue
  \global\WI@brokefalse
  \WI@pendingfalse
}

\newcommand\WI@deactivate{%
  \WI@checkpending
  \global\advance\WI@activecount\m@ne
  \ifnum\WI@activecount>0\relax
  \else
...
\newenvironment{warnitemize}
  {\WI@activate\begin{itemize}}
  {\end{itemize}\WI@deactivate}
...
\NewDocumentEnvironment{warnmulticol}{O{\WI@defaultmaxcols} o}
  {%
    \WI@starfalse
...
\NewDocumentEnvironment{warnmulticolitemize}{O{\WI@defaultmaxcols} o}
  {%
    \IfNoValueTF{#2}{\begin{warnmulticol}[#1]}{\begin{warnmulticol}[#1][#2]}%
    \begin{itemize}%
  }
  {%
    \end{itemize}%
...
\NewDocumentEnvironment{warnmulticolenumerate}{O{\WI@defaultmaxcols} o}
  {%
    \IfNoValueTF{#2}{\begin{warnmulticol}[#1]}{\begin{warnmulticol}[#1][#2]}%
    \begin{enumerate}%
  }
...
\NewDocumentEnvironment{warnmulticolcheckboxes}{O{\WI@defaultmaxcols} o}
  {%
    \IfNoValueTF{#2}{\begin{warnmulticol}[#1]}{\begin{warnmulticol}[#1][#2]}%
    \begin{checkboxes}%
  }
...
\newcommand\WI@columnspread@begin[2]{%
  \ifWI@columnspread
    \PackageError{warnitemize}{Nested columnspread environments are unsupported}{%
...
The goals would be to keep using the standard itemize environment, and to use etoolbox-like patching to monkey-patch the logic into it.

...
\makeatletter
% ------------------------------------------------------------------
% columnspread user guide (keep in sync with README once packaged)
% ------------------------------------------------------------------
% - Register environments by name:
%     \columnspreadfor{itemize,enumerate}
%   or patch one at a time:
%     \ColumnSpreadPatch{choices}
%
% - columnspread keys:
%     \begin{columnspread}[<max cols>][<key>]
%   The optional key (e.g. [5][Q1]) persists column widths across runs
%   and is suffixed automatically (Q1-1, Q1-2, …) per top-level list.
%
% - Only the first list level inside the environment is spread; nested
%   sub-lists stay single column. Avoid verbatim-like content, minted,
%   lstlisting, or floats inside items; they can break detection.
%
% - If column counts oscillate between compilations, clear aux files
%   and compile twice so cached column totals stabilise.
%
% - Floats (figure/table) inside spread items are discouraged; a warning
%   is emitted when they are detected and layout may degrade.
% ------------------------------------------------------------------
\newif\ifWI@pending
\newif\ifWI@broke
\newif\ifWI@rerun
...
\providecommand\columnspread@optiondefaults{}
\newcommand\columnspread@applydefaults[1]{%
...
\providecommand\columnspread@optionsprocessed{}%
\ifx\columnspread@optionsprocessed\@empty
  \DeclareOption{defaults=exam}{\columnspread@applydefaults{exam}}
  \DeclareOption*{%
    \PackageWarning{columnspread}{Unknown option '\CurrentOption'}%
  }
  \ProcessOptions\relax
  \let\columnspread@optionsprocessed\relax
\fi
